...
const BaseNodeSchema = z.object({
  id: z
    .string()
    .describe("The id of the node as a number without a leading zero."),
  label: z.string(),
...
export const EdgeSchema = z.object({
  source: z
    .string()
    .describe(
      "The id of the source node. Do not include leading zeros if using numbers.",
    ),
  target: z
    .string()
    .describe(
      "The id of the target node. Do not include leading zeros if using numbers.",
    ),
  label: z
    .string()
    .describe(`The label of the edge. Allowed values: ${edgeTypes.join(", ")}`),
});
